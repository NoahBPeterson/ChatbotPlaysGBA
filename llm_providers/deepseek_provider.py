"""
DeepSeek provider implementation for the GameAgent.
"""
...
# Configure logging
logger = logging.getLogger('game_agent.deepseek_provider')


class DeepSeekProvider(LLMProvider):
    """DeepSeek API provider for LLM interactions (compatible with OpenAI API)"""
    ...
    def __init__(self, api_key: str, model: str = "deepseek-chat"):
        ...
        # Import here to avoid dependency if not using DeepSeek
        try:
            import openai
    ...
    def generate_response(self, prompt: str, context: List[Dict[str, str]]) -> str:
        """Generate a response using DeepSeek API (OpenAI compatible)"""
        try:
            ...
            try:
                ...
                logger.debug("Received successful response from DeepSeek API")
                ...
                logger.warning(f"Function calling failed: {e}. Falling back to standard completion.")
                
                # Fallback to standard chat completion without function calling
                response = self.client.chat.completions.create(
                    model=self.model,
                    messages=messages,
                    max_tokens=1000
                ...
                logger.debug("Received fallback response from DeepSeek API")
